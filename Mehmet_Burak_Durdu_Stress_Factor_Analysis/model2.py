...
class StressLevelModel:
    ...
    def apply_pca(self, n_components):
            ...
            """
            original_linear_cols = self.linear_cols.copy()  # Copy the original linear columns
            pca = PCA(n_components=n_components)
            X_linear = self.df[self.linear_cols]
            X_linear_pca = pca.fit_transform(X_linear)
            
            # Create new column names for the PCA components
            pca_column_names = [f'pca_{i+1}' for i in range(n_components)]
            
            # Replace the original linear columns with the PCA components
            self.df.drop(columns=self.linear_cols, inplace=True)
            self.df[pca_column_names] = X_linear_pca
            print(f"Applied PCA to reduce dimensionality. Number of components kept: {n_components}")
            
            # Update linear_cols to contain the new PCA component names
            self.linear_cols = pca_column_names
            
            # Print the columns used for PCA and the columns that were dropped
            print(f"Columns used for PCA: {original_linear_cols}")
            print(f"Columns dropped after PCA: {list(self.df.columns.difference(original_linear_cols))}")
    ...
    def run(self):
        ...
        # Apply PCA for dimensionality reduction before splitting for linear regression
        self.apply_pca(n_components=10)  # Adjust the number of components as needed
